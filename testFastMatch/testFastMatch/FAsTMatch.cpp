...
namespace fast_match {
	 ...
	 */
	vector<cv::Point2f> FAsTMatch::apply(cv::Mat& original_image, cv::Mat& original_template) {
		...
		/* Smooth our images */
		GaussianBlur(templ, templ, cv::Size(0, 0), 2.0, 2.0);
		GaussianBlur(image, image, cv::Size(0, 0), 2.0, 2.0);
		...
		while (true) {
			...
			vector<cv::Mat> affines = configsToAffine(configs, insiders);
	 ...
	 */
	vector<MatchConfig> FAsTMatch::createListOfConfigs(MatchNet& net, cv::Size templ_size, cv::Size image_size) {
	...
	vector<MatchConfig> FAsTMatch::randomExpandConfigs(vector<MatchConfig>& configs, MatchNet& net, int level,
		...
		vector<cv::Mat> configs_mat(no_of_configs);
		...
		cv::Mat ranges = repeat(cv::Mat(ranges_vec).t(), no_of_points * no_of_configs, 1);
	 ...
	 **/
	vector<cv::Mat> FAsTMatch::configsToAffine(vector<MatchConfig>& configs, vector<bool>& insiders) {
		int no_of_configs = static_cast<int>(configs.size());
		vector<cv::Mat> affines(no_of_configs);

		/* The boundary, between -10 to image size + 10 */
		cv::Point2d top_left(-10., -10.);
		cv::Point2d bottom_right(image.cols + 10, image.rows + 10);
	 ...
	 */
	vector<double> FAsTMatch::evaluateConfigs(cv::Mat& image, cv::Mat& templ, vector<cv::Mat>& affine_matrices,
		...
		cv::Mat padded(image.rows * 3, image.cols, image.type(), cv::Scalar(0.0));
		image.copyTo(cv::Mat(padded, cv::Rect(0, image.rows, image.cols, image.rows)));
		...
		cv::Mat xs_centered = xs.clone() - (r1x + 1),
	 ...
	 **/
	cv::Mat FAsTMatch::preprocessImage(cv::Mat& image) {
	 ...
	 * rectangle
	 */
	vector<cv::Point2f> FAsTMatch::calcCorners(cv::Size image_size, cv::Size templ_size, cv::Mat& affine) {
