...
namespace fast_match {
    ...
    vector<MatchConfig> MatchConfig::fromMatrix(cv::Mat& configs) {
     ...
     */
	cv::Mat MatchConfig::asMatrix() {
        return (cv::Mat_<float>(1, 6) << translateX, translateY, rotate2, scaleX, scaleY, rotate1 );
    }
    
    /**
     * Returns the affine matrix representation of the configuration
     */
	cv::Mat MatchConfig::getAffineMatrix() {
        return this->affine;
    }
     ...
     * Create an affine transformation matrix from the configurations
     */
	cv::Mat MatchConfig::asAffineMatrix() {
